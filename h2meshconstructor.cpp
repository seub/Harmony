...
void H2MeshConstructor::createCutMeshPoints()
{
    ...
    for (i=0; i<nbCuts; i++)
    {
        ...
        (*(((*subdivisions)[cut.leftTriangleIndex]).meshIndices))[indicesLeft[0]] = nextIndex;
        (*(((*subdivisions)[cut.rightTriangleIndex]).meshIndices))[indicesRight[0]] = nextIndex;
        nextIndex++;
        ...
        (*(((*subdivisions)[cut.leftTriangleIndex]).meshIndices))[indicesLeft.back()] = nextIndex;
        (*(((*subdivisions)[cut.rightTriangleIndex]).meshIndices))[indicesRight.back()] = nextIndex;
        nextIndex++;
    }
...
void H2MeshConstructor::createSideMeshPoints()
{
    ...
    for (i=0; i<nbVertices; i++)
    {
        ...
        if (!vertexAdded[i])
        {
            ...
            (*(((*subdivisions)[triangleIndex]).meshIndices))[indices[0]] = nextIndex;
            nextIndex++;
        }
